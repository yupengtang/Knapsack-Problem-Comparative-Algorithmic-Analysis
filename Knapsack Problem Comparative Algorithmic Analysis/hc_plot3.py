...
timeList = np.arange(0, 15, 1)  # Adjust upper bound and increment as needed
print(len(timeList))
...
print(len(timeList))
respath = "./DATA/DATASET/trace_file/large_scale/large_3_LS1"
...
if __name__=="__main__":
	...
	print(fileToReads)
	resTable = np.zeros((15,7),dtype=float)
	...
	t3 = resTable[5,:]/100
	t4 = resTable[7,:]/100
	plt.plot(quality*100,t1,'-r',label="1s")
	plt.plot(quality*100,t2,'-b',label="3s")
	plt.plot(quality*100,t3,'-m',label="5s")
	plt.plot(quality*100,t4,'-y',label="7s")
	plt.legend()
