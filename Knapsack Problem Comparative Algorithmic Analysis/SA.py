...
def SA_once(N,W,valueArr,weightArr,cutTime,seed,filename):
	np.random.seed(seed)
	initProb = W/weightArr.sum()/2
	initArr = np.random.binomial(1, initProb, size=N)
	
	instanceName = os.path.basename(filename)
	instanceDir = os.path.dirname(filename)
	instanceDir = os.path.join(instanceDir,instanceName+"_SA")
	instanceName = instanceName +"_"+"SA_"+str(cutTime) + "_"+str(seed) 
	...
	while(True):
		...
		if(timePast>=cutTime):
			print("timeout,SA done",)
			break
		neighborItems = None
		while(True):
			index = np.random.randint(N)
			neighborItems = curItems.copy()
			neighborItems[index] = 1-neighborItems[index]
			if(neighborItems@weightArr<=W):
				break
		neighborVal = neighborItems@valueArr
		if(neighborVal>curValue):
			curValue = neighborVal
			curItems = neighborItems.copy()
			if(curValue>bestVal):
				bestVal=curValue
				bestItems=curItems.copy()
				trace.append([(time.time()-initTime)*1000,bestVal])
		else:
			prob = np.exp(K*(neighborVal-curValue)/initT)
...
def SA(fileName,cutOff,seed):
	N,W,valueList,weightList = readFile(fileName)
	bestVal,bestItems = SA_once(N,W,np.array(valueList,dtype=float),np.array(weightList,dtype=float),cutOff,seed,fileName)
